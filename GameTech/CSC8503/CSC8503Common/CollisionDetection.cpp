...
bool CollisionDetection::RayIntersection(const Ray& r,GameObject& object, RayCollision& collision) {
	...
	if (volume->lType != layerType::Default) { return false; }
...
bool CollisionDetection::AABBSphereIntersection(const AABBVolume& volumeA, const Transform& worldTransformA,
	...
	//	//Vector3 closestPointOnBoxSurface = Maths::Clamp(delta * 1.5f, -boxSize, boxSize);

	//	//Vector3 localPoint2 = delta - closestPointOnBoxSurface;
	//	//float distance2 = (localPoint2).Length();
	//	//bool a = true;

	//	//collisionInfo.AddContactPoint(closestPointOnBoxSurface + worldTransformA.GetWorldPosition(), delta.Normalised(), distance2 - volumeB.GetRadius());

	//	//int index = 0;

	//	//if(abs(d))

	//	//float realDistance= Vector3::Dot(
	...
	if (distance < volumeB.GetRadius()) {// yes , we ’re colliding !
		collisionInfo.AddContactPoint( closestPointOnBox + worldTransformA.GetWorldPosition(), distance == 0.001f ? delta.Normalised() : localPoint.Normalised(),(volumeB.GetRadius() - distance));
		return true;
	}
