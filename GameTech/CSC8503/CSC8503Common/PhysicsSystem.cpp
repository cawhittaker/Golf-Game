...
PhysicsSystem::PhysicsSystem(GameWorld& g) : gameWorld(g)	{
	...
	dTOffset		= 0.0f;
	globalDamping	= 0.05f;
	SetGravity(Vector3(0.0f, -300.8f, 0.0f));
...
void PhysicsSystem::Update(float dt) {
	const float iterationDt = 1.0f / 500.0f; //Ideally we'll have 120 physics updates a second 
...
void PhysicsSystem::UpdateCollisionList() {
	for (std::set<CollisionDetection::CollisionInfo>::iterator i = allCollisions.begin(); i != allCollisions.end(); ) {
		if ((*i).framesLeft == numCollisionFrames) {
			...
			i->a->OnCollisionBegin2(i->b);
			i->b->OnCollisionBegin2(i->a);
		}
...
void PhysicsSystem::ImpulseResolveCollision(GameObject& a, GameObject& b, CollisionDetection::ContactPoint& p) const {
	...
	//std::cout << "Collision normal: " << p.normal << std::endl;
...
void PhysicsSystem::IntegrateVelocity(float dt) {
	...
	std::vector < GameObject * >::const_iterator last;
	gameWorld.GetObjectIterators(first, last);
	float dampingFactor = 1.0f - 0.55f;
	float frameDamping = powf(dampingFactor, dt);
	
		for (auto i = first; i != last; ++i) {
		PhysicsObject * object = (*i) -> GetPhysicsObject();
		if (object == nullptr) {continue;}
		Transform & transform = (*i) -> GetTransform();// Position Stuff

		Vector3 position = transform.GetLocalPosition();
		Vector3 linearVel = object -> GetLinearVelocity();
		position += linearVel * dt;
		transform.SetLocalPosition(position); // Linear Damping
		transform.SetWorldPosition(position); // slide change
